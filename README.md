...
送信バー左の「…」ボタンからログ機能にアクセスできます。

- **自動保存**：キャラクターを選択するたびに新しいログセッションが開始され、`log/` フォルダに自動保存されます。
- **ログを保存**：現在のセッションのログを任意のフォルダ・ファイル名で保存します（デフォルト: `output/`）。ファイル名を空白にすると自動生成されます。
- **ログを読み込む**：保存済みの `.log` ファイルを選択して会話を再表示します。読み込んだ吹き出しには ↻（音声再生成）と ⋯（ここからリプレイ）ボタンが付きます。
