...
1キャラクターにつき **名前の一致した `.txt`（プロンプト）と `.wav`（サンプル音声）** が必要です。画像（`.png` など）は任意です。.txtのみの追加でキャラクターを定義する事も可能ですが、その場合音声生成非対応キャラとして認識されます。
...
| 項目 | 機能 |
|---|---|
| ▶▶ ここからリプレイ | その吹き出し以降の音声を順番に連続再生します。再生中は送信ボタンが ■ に変わり、クリックで停止できます |
